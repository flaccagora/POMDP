...
public:
    ...
    Vector2 transform(const Vector2& position, const std::optional<Matrix2x2>& custom_matrix = std::nullopt) const {
        if (custom_matrix) {
            return matrix_vector_multiply(*custom_matrix, position);
        }
...
PYBIND11_MODULE(gridworld, m) {
    try {
        py::class_<ObservableDeformedGridworld>(m, "ObservableDeformedGridworld", py::module_local())
            ...
            .def("transform", &ObservableDeformedGridworld::transform, py::arg("position"), py::arg("custom_matrix") = py::none())
            ...
            .def_readonly("grid_size", &ObservableDeformedGridworld::grid_size_);
    } catch (const std::exception& e) {
        // If the class is already registered, we can safely ignore the error
